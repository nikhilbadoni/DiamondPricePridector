...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        try:
            ...
            drop_columns = [target_cloumn_name,'id']

            input_feature_train_df = test_df.drop(drop_columns, axis= 1)
            target_feature_train_df = test_df[target_cloumn_name]
            
            input_feature_test_df = test_df.drop(drop_columns, axis= 1)
            target_feature_test_df = test_df[target_cloumn_name]

            # Transforming using preprocessor object
            input_feature_train_arr = preprocessing_obj.fit_transform(input_feature_train_df)
            input_feature_test_arr = preprocessing_obj.transform(input_feature_test_df)

            logging.info("Applying Preprocessing Object ontraining and testing Datasets...")
